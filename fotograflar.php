				...
				<!-- Main -->
					<div id="main">
						...
						<?php
							...
							$yazilari_secme2 = mysqli_query($con, "SELECT * FROM `yazilar` WHERE `kategori` = '6' ORDER BY `yazilar`.`tarih` DESC ");
							...
							$yazilari_secme = mysqli_query($con, "SELECT * FROM `yazilar` WHERE `kategori` = '6' ORDER BY `yazilar`.`tarih` DESC LIMIT ".$how_many." OFFSET ".$from_where."");
						...
						<!-- Pagination -->
							<ul class="actions pagination">
								<li><a href="fotograflar.php?page=<?php echo $page - 1 ?>" class="<?php if($page == 1){echo "disabled ";}else {echo " ";} ?>button big previous">Önceki Sayfa</a></li>
								<li><a href="fotograflar.php?page=<?php echo $page + 1 ?>" class="<?php if($page*$how_many > $yazilarin_sayisi){echo "disabled ";}else {echo "";} ?>button big next">Sonraki Sayfa</a></li>
							</ul>
